...
class GetSalesLists(object):
    ...
    def specialty_food(self):
        self.specialty_food_html=[]
        log("grabbing initial 'specialty_food' page")
        self.driver.get('http://www.yelp.com/search?find_desc=Specialty+Food&find_loc=Chicago%2C+IL&ns=1')
        self.grocery_html.append(self.driver.page_source)
    ...
    def farmersmarket(self):
        ...
        log("grabbing initial 'farmersmarket' page")
        self.driver.get('http://www.yelp.com/search?find_desc=Farmers+Market&find_loc=Chicago%2C+IL&ns=1')
        self.grocery_html.append(self.driver.page_source)
        ...
        for i in range(10,200,10):
            ...
            time.sleep(num)
            log('getting another link')
            link="http://www.yelp.com/search?find_desc=Farmers+Market&find_loc=Chicago%2C+IL&ns=1#start="+str(i)
            self.driver.get(link)
            self.farmersmarket_html.append(self.driver.page_source)

    def run_all(self):
        self.grocery()
        self.specialty_food()
        self.farmersmarket()
        self.get_names(self.grocery_html)
        self.get_names(self.specialty_food_html)
        self.get_names(self.farmersmarket_html)


    def get_names(self,from_where):
        for html in from_where:
        ...
        self.company_names=set(self.company_names) #saves on mem space
